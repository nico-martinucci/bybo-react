...
function ListingDetail({ addUserBooking }) {
    ...
    if (listing.isLoading) return <LoadingPage />;
    ...
    return (
        <Container>
            <h3>{l.name}</h3>
            <p>hosted by <Link to={`/users/${l.host.id}`}>
                {l.host.username}</Link>  | {l.location}</p>
            <Card.Img className="mb-2" variant="left" width="600px" src={l.photo}/>
            <p style={{fontSize: "1.2em"}}>{l.description}</p>
            <p>
                {l.has_pool && "Pool! "}
